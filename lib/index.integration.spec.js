...
describe('json-post-process-webpack-plugin (integration)', () => {
  describe('adding a property, with synchronous action', () => {
    ...
    it('should add a property to manifest.json', () => {
      ...
      expect(contents).to.eql({
        '1.1-fdd320b564c976534b4c.js': 'http://cdn.com/assets/1.1-fdd320b564c976534b4c.js',
        '2.2-fdd320b564c976534b4c.js': 'http://cdn.com/assets/2.2-fdd320b564c976534b4c.js',
        'main.js': 'http://cdn.com/assets/main-fdd320b564c976534b4c.js',
        added: 'a property'
      })
    })
  })
  ...
  describe('adding a property for the matched assetKey, with synchronous action', () => {
    beforeEach(function (done) {
      const config = setupWithMatchers([{
        matcher: /^manifest.json$/,
        action: (json, assetKey) => Object.assign({}, json, { [assetKey]: 'modified!' })
      }])
      webpack(config, done)
    })
  ...
  describe('returning new content, with Promise action', () => {
    beforeEach(function (done) {
      const config = setupWithMatchers([{
        matcher: /^manifest.json$/,
        action: (json, assetKey) => Promise.resolve({ something: 'completely different' })
      }])
      webpack(config, done)
    })
